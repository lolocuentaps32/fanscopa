...
const AdminDashboard: React.FC = () => {
  ...
  const COLORS = ['#10b981', '#f59e0b', '#0D88CA', '#ef4444'];
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-500 font-medium text-sm">Cargando base de datos...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-slate-800 uppercase tracking-tight">Panel de Control <span className="text-[#0D88CA]">Albali</span></h1>
        <button
          onClick={handleAnalyze}
          disabled={loadingAnalysis}
          className="bg-[#0D88CA] hover:bg-[#0c7ab5] text-white px-6 py-2 rounded-lg font-semibold transition disabled:opacity-50 flex items-center gap-2"
        >
          {loadingAnalysis ? (
      ...
      {analysis && (
        <div className="bg-sky-50 border border-sky-100 p-6 rounded-xl shadow-sm animate-fade-in">
          <h2 className="text-xl font-semibold text-[#0D88CA] mb-2">Resumen Inteligente</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white p-4 rounded-lg shadow-sm border border-sky-100">
              <span className="block text-sm font-medium text-slate-500">Porcentaje Abonados</span>
              <span className="text-2xl font-bold text-[#0D88CA]">{(analysis.metrics.abonadosPerc * 100).toFixed(1)}%</span>
            </div>
            <div className="bg-white p-4 rounded-lg shadow-sm border border-sky-100">
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                {analysis.metrics.topLocations.map((loc: string) => (
                  <span key={loc} className="px-2 py-1 bg-sky-100 rounded text-xs text-[#0D88CA] font-medium">{loc}</span>
